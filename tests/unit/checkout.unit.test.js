...
describe("Cart test", () => {
  ...
  describe("Checkout", () => {
    ...
    it("should throw 404 error if cart is not present", async () => {
      ...
      // Call the cartService.checkout method with userOne and store the result in res
      ...
      // Assert that an ApiError is thrown and it contains the correct status code
    ...
    it("should throw 400 error if address is not set - when User.hasSetNonDefaultAddress() returns false", async () => {
      // Assert that userTwo's address is the default address
      expect(userTwo.address).toEqual(config.default_address);

      // Mock Cart model to return `cartWithProductsUserTwo` as output to `Cart.findOne()` call
      ...
      // Call the cartService.checkout method with userTwo and store the result in res
    ...
    it("should throw 400 error if wallet balance is insufficient", async () => {
      // Mock Cart model to return `cartWithProductsUserOne` as output to `Cart.findOne()` call
      mockingoose(Cart).toReturn(cartWithProductsUserOne, "findOne");

      // Create a user object with zero wallet balance
      ...
      // Call the cartService.checkout method with userOneWithZeroBalance and store the result in res
    ...
    it("should update user balance and empty the cart on success", async () => {
      // Create a copy of userOne for testing
